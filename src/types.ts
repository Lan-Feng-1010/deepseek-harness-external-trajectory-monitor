...
export interface HarnessSession {
  readonly id: string
  append(type: string, data: JsonObject, envelope?: JsonObject): { readonly seq: number }
}
...
export interface HarnessContext {
  ...
  readonly sessionPersistence: { list(): Promise<readonly { id: string }[]> }
  readonly sessions: {
    create(id: string, options: JsonObject): HarnessSession
    flush(session: HarnessSession): Promise<void>
  }
  readonly tools: { register(definition: JsonObject): void }
  readonly webServer: {
    register(route: {
