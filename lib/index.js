...
import { appendFile, cp, mkdir, open, readFile, readdir, rename, stat, writeFile } from "node:fs/promises";
...
function requireSafeRelativePath(value, label) {
	...
	if (parsed.replaceAll("\\", "/").split("/").some((part) => part === "..")) throw new Error(`${label} must not traverse parent directories`);
	return parsed;
}
...
async function readJson$1(path, label) {
	let text;
	try {
...
function validateRuntimeRegistration(value, label) {
	if (value.schemaVersion !== 1 || !Array.isArray(value.sources)) throw new Error(`${label} must use schemaVersion 1 and contain sources`);
...
function validateCase(raw, label) {
...
function validateSourceTemplate(raw, label) {
	const value = requireObject(raw, label);
	...
	if (![
		...
		"generic",
		"implantagent-trace"
	].includes(kind)) throw new Error(`${label}.kind is unsupported`);
	const projectionMode = value.projectionMode === void 0 ? void 0 : value.projectionMode === "implantagent-modules" || value.projectionMode === "default" ? value.projectionMode : (() => {
	...
	return {
		...
		...value.cwd === void 0 ? {} : { cwd: requireString(value.cwd, `${label}.cwd`) },
		...value.suffix === void 0 ? {} : { suffix: requireString(value.suffix, `${label}.suffix`) },
		nativeSession: value.nativeSession !== false,
		...value.provider === void 0 ? {} : { provider: requireString(value.provider, `${label}.provider`) },
		...value.model === void 0 ? {} : { model: requireString(value.model, `${label}.model`) },
		ledgerRoot: requireString(value.ledgerRoot, `${label}.ledgerRoot`),
		...projectionMode === void 0 ? {} : { projectionMode }
	};
}
function validatePlan(raw, index) {
	...
	const maxConcurrentRuns = value.maxConcurrentRuns === void 0 ? 1 : Number(value.maxConcurrentRuns);
	if (!Number.isSafeInteger(maxConcurrentRuns) || maxConcurrentRuns < 1 || maxConcurrentRuns > 8) throw new Error(`${label}.maxConcurrentRuns must be an integer from 1 to 8`);
	...
	return {
		...
		maxConcurrentRuns,
		requiredPaths: requireStringArray(value.requiredPaths ?? [], `${label}.requiredPaths`).map((item, pathIndex) => requireSafeRelativePath(item, `${label}.requiredPaths[${pathIndex}]`)),
		requiredEmptyDirectories: requireStringArray(value.requiredEmptyDirectories ?? [], `${label}.requiredEmptyDirectories`).map((item, pathIndex) => requireSafeRelativePath(item, `${label}.requiredEmptyDirectories[${pathIndex}]`)),
		completionPaths: requireStringArray(value.completionPaths ?? [], `${label}.completionPaths`).map((item, pathIndex) => requireSafeRelativePath(item, `${label}.completionPaths[${pathIndex}]`)),
		cases,
		command: {
...
async function loadLaunchPlans(path) {
	const manifest = await readJson$1(path, "external preexperiment launch plan manifest");
	if (manifest.schemaVersion !== 1 || !Array.isArray(manifest.plans)) throw new Error("external preexperiment launch plan manifest must use schemaVersion 1 and contain plans");
...
function expandTemplate(value, variables, label) {
	const expanded = value.replace(TEMPLATE_TOKEN, (_match, key) => variables[key]);
	if (/\{[^{}]+\}/.test(expanded)) throw new Error(`${label} contains an unsupported template token`);
	return expanded;
}
function expandSource(source, variables, index) {
	...
	if (!isInside(variables.run_root, root) || !isInside(variables.run_root, cwd) || !isInside(variables.run_root, ledgerRoot)) throw new Error(`${label} paths must stay inside the fresh run root`);
	return {
		id,
...
async function validateTemplate(caseSpec, plan) {
	if (!(await stat(caseSpec.templateRoot)).isDirectory()) throw new Error(`template root is not a directory: ${caseSpec.templateRoot}`);
	...
	for (const relativePath of plan.requiredEmptyDirectories) {
		...
		if (!(await stat(candidate)).isDirectory()) throw new Error(`required empty path is not a directory: ${relativePath}`);
		if ((await readdir(candidate)).length !== 0) throw new Error(`template runtime directory is not empty: ${relativePath}`);
	}
}
function managedRunId(planId, caseId) {
...
async function missingCompletionPaths(runRoot, completionPaths) {
	const missing = [];
...
async function loadRunRecords(root) {
	let entries;
	try {
	...
	for (const entry of entries.filter((item) => item.isFile() && item.name.endsWith(".run.json")).sort((left, right) => left.name.localeCompare(right.name))) {
		...
		const value = await readJson$1(join(root, entry.name), label);
...
async function loadRuntimeRegistrations(root) {
	let entries;
	try {
		entries = await readdir(root, { withFileTypes: true });
	} catch (error) {
	...
	for (const entry of entries.filter((item) => item.isFile() && item.name.endsWith(".registration.json")).sort((left, right) => left.name.localeCompare(right.name))) {
		...
		registrations.push(validateRuntimeRegistration(await readJson$1(join(root, entry.name), label), label));
	}
	return registrations;
}
var PreexperimentLaunchManager = class {
	options;
	...
	async catalog() {
		return {
			schemaVersion: 1,
			plans: (await loadLaunchPlans(this.options.plansPath)).plans.filter((plan) => plan.enabled).map((plan) => ({
				id: plan.id,
				label: plan.label,
	...
	async status(args = {}) {
		const selected = (await loadRunRecords(this.options.runRegistryRoot)).filter((record) => (args.run_id === void 0 || record.runId === args.run_id) && (args.plan_id === void 0 || record.planId === args.plan_id) && (args.case_id === void 0 || record.caseId === args.case_id));
		const registrations = await loadRuntimeRegistrations(this.options.registrationRoot);
		return {
			schemaVersion: 1,
			runs: await Promise.all(selected.map(async (record) => {
	...
	async start(args) {
		...
		try {
			const plan = (await loadLaunchPlans(this.options.plansPath)).plans.find((item) => item.id === args.plan_id && item.enabled);
			if (plan === void 0) throw new Error(`enabled launch plan not found: ${args.plan_id}`);
			const caseSpec = plan.cases.find((item) => item.caseId === args.case_id);
			if (caseSpec === void 0) throw new Error(`case is not allowlisted for plan ${plan.id}: ${args.case_id}`);
			...
			if (activeManagedRuns.length > 0) throw new Error(`managed run ${activeManagedRuns[0]?.runId ?? "unknown"} is still running; wait for it to finish before starting another external agent`);
			if (records.filter((record) => record.planId === plan.id && record.state === "running" && processAlive(record.processId) === true).length >= plan.maxConcurrentRuns) throw new Error(`launch plan ${plan.id} reached its concurrent-run limit`);
			...
			await cp(caseSpec.templateRoot, runRoot, {
				recursive: true,
				errorOnExist: true,
				force: false,
			...
			await writeJsonAtomic(registrationPath, {
				schemaVersion: 1,
				runId,
				planId: plan.id,
			...
			if (!(await stat(executable)).isFile()) throw new Error(`configured executable is not a file: ${executable}`);
			if (!(await stat(commandCwd)).isDirectory()) throw new Error(`configured command cwd is not a directory: ${commandCwd}`);
			...
			child.once("error", (error) => {
				...
				writeJsonAtomic(recordPath, failed);
			});
			child.once("exit", (code, signal) => {
				...
				(async () => {
					...
					const finished = {
						...
						exitCode: code,
						signal,
						error: completed ? null : code === 0 ? `trusted supervisor exited with 0 but completion artifact(s) are missing: ${missing.join(", ")}` : `trusted supervisor exited with ${code ?? signal ?? "unknown status"}`
					};
...
const name = "external-trajectory-importer";
const inject = [
	"sessions",
	"sessionPersistence",
	"webServer",
...
const LIVE_MONITOR_SESSION_ID = `${SESSION_PREFIX}live-monitor-v4-3`;
const NATIVE_LIVE_PROJECTION_VERSION = "4.3.0";
const NORMALIZED_LEDGER_SCHEMA = "external-trajectory-ledger-v3";
const TRACE_ROUTE = "/api/external-reasoning-trace";
const TOOL_ITEM_TYPES = /* @__PURE__ */ new Set([
	"command_execution",
	"file_change",
	"web_search",
	"mcp_tool_call"
...
const IMPLANTAGENT_TOOLS = Object.freeze({
	t01_segmentation_selection: {
		nodeId: "T01",
		moduleId: "M1",
		title: "分割选择 / Segmentation selection"
	},
	t02_candidate_generation: {
		nodeId: "T02",
		moduleId: "M2",
		title: "候选位点生成 / Candidate generation"
	},
	t03_admission: {
		nodeId: "T03",
		moduleId: "M2",
		title: "位点准入 / Site admission"
	},
	t04_routing: {
		nodeId: "T04",
		moduleId: "M2",
		title: "路径证据 / Routing evidence"
	},
	t05_initial_entry: {
		nodeId: "T05",
		moduleId: "M3",
		title: "初始入口 / Initial entry"
	},
	t06_initial_axis: {
		nodeId: "T06",
		moduleId: "M3",
		title: "初始轴向 / Initial axis"
	},
	t07_initial_size: {
		nodeId: "T07",
		moduleId: "M3",
		title: "初始尺寸 / Initial size"
	},
	t08_initial_depth: {
		nodeId: "T08",
		moduleId: "M3",
		title: "初始深度 / Initial depth"
	},
	t09_sinus_path: {
		nodeId: "T09",
		moduleId: "M4",
		title: "上颌窦路径 / Sinus path"
	},
	t10_anatomic_safety: {
		nodeId: "T10",
		moduleId: "M4",
		title: "解剖安全 / Anatomic safety"
	},
	t11_refinement: {
		nodeId: "T11",
		moduleId: "M4",
		title: "有界修正 / Bounded refinement"
	},
	t12_multisite_coordination: {
		nodeId: "T12",
		moduleId: "M5",
		title: "多位点协调 / Multi-site coordination"
	},
	t13_output_package: {
		nodeId: "T13",
		moduleId: "M6",
		title: "输出封装 / Output package"
	}
});
const IMPLANTAGENT_NODE_TITLES = Object.freeze({
	T01: "分割选择 / Segmentation selection",
	T02: "候选位点生成 / Candidate generation",
	T03: "位点准入 / Site admission",
	T04: "路径证据 / Routing evidence",
	T05: "初始入口 / Initial entry",
	T06: "初始轴向 / Initial axis",
	T07: "初始尺寸 / Initial size",
	T08: "初始深度 / Initial depth",
	T09: "上颌窦路径 / Sinus path",
	T10: "解剖安全 / Anatomic safety",
	T11: "有界修正 / Bounded refinement",
	T12: "多位点协调 / Multi-site coordination",
	T13: "输出封装 / Output package"
});
...
function validateSource(source, index) {
	if (source === null || typeof source !== "object" || Array.isArray(source)) throw new Error(`sessions[${index}] must be an object`);
	const agent = requireNonEmptyString(source.agent, `sessions[${index}].agent`);
	if (agent !== "codex" && agent !== "claude") throw new Error(`sessions[${index}].agent must be "codex" or "claude"`);
	const checked = {
		agent,
	...
	if (!isAbsolute(checked.cwd) || !isAbsolute(checked.sourcePath) || !isAbsolute(checked.runRecordPath)) throw new Error(`sessions[${index}] cwd and source paths must be absolute`);
	if (!/^[a-f0-9]{64}$/.test(checked.expectedSha256)) throw new Error(`sessions[${index}].expectedSha256 must be a lowercase SHA-256 digest`);
	return checked;
}
async function loadManifest(path = DEFAULT_MANIFEST_PATH) {
	const manifest = await readJson(path, "trajectory source manifest");
	if (manifest?.schemaVersion !== 1 || !Array.isArray(manifest.sessions)) throw new Error("trajectory source manifest must use schemaVersion 1 and contain a sessions array");
	return {
		schemaVersion: 1,
		sessions: manifest.sessions.map(validateSource)
	};
}
function validateLiveSource(source, index) {
	if (source === null || typeof source !== "object" || Array.isArray(source)) throw new Error(`live sources[${index}] must be an object`);
	...
	if (![
		...
		"generic",
		"implantagent-trace"
	].includes(kind)) throw new Error(`live sources[${index}].kind is unsupported`);
	...
	const cwd = source.cwd === void 0 ? root : requireNonEmptyString(source.cwd, `live sources[${index}].cwd`);
	...
	return {
		...
		provider: typeof source.provider === "string" && source.provider !== "" ? source.provider : kind === "claude" ? "claude-cli" : kind === "codex" ? "codex-cli" : "external-process",
		model: typeof source.model === "string" && source.model !== "" ? source.model : kind === "claude" ? "external-claude" : kind === "codex" ? "external-codex" : "external-agent",
		ledgerRoot: source.ledgerRoot === void 0 ? DEFAULT_LEDGER_ROOT : resolve(requireNonEmptyString(source.ledgerRoot, `live sources[${index}].ledgerRoot`)),
		projectionMode: source.projectionMode === "implantagent-modules" ? "implantagent-modules" : "default"
	};
}
async function loadLiveManifest(path = DEFAULT_LIVE_MANIFEST_PATH) {
	const manifest = await readJson(path, "live trajectory source manifest");
	if (manifest?.schemaVersion !== 1 || !Array.isArray(manifest.sources)) throw new Error("live trajectory source manifest must use schemaVersion 1 and contain a sources array");
	return {
		schemaVersion: 1,
		sources: manifest.sources.map(validateLiveSource)
	};
}
async function loadCombinedLiveManifest(path = DEFAULT_LIVE_MANIFEST_PATH, runtimeRegistrationRoot) {
	const base = await loadLiveManifest(path);
	if (runtimeRegistrationRoot === void 0) return base;
	const registrations = await loadRuntimeRegistrations(runtimeRegistrationRoot);
	const sources = [...base.sources];
	const ids = new Set(sources.map((source) => source.id));
	for (const registration of registrations) for (const [index, rawSource] of registration.sources.entries()) {
...
function codexToolArguments(item) {
	switch (item.type) {
		...
		case "web_search": return JSON.stringify({
			query: item.query,
			action: item.action
		});
		case "mcp_tool_call": return JSON.stringify(item.arguments ?? {});
...
function codexToolResult(item) {
	...
	if (item.type === "mcp_tool_call") {
		if (item.result !== void 0) return JSON.stringify(item.result);
		if (item.error !== void 0) return JSON.stringify(item.error);
	}
...
function implantagentToolMetadata(item) {
	...
	const metadata = IMPLANTAGENT_TOOLS[String(item.tool)];
...
function codexToolDisplayName(item) {
	...
	const tracebackScript = typeof item.aggregated_output === "string" ? item.aggregated_output.match(/File\s+["']([^"']+\.py)["']/i)?.[1] : void 0;
	if (tracebackScript !== void 0) return pathLeaf(tracebackScript);
	const quotedPython = command.match(/["']([^"']+\.py)["']/i);
	const plainPython = command.match(/(?:^|\s)([^\s;|&]+\.py)(?=$|\s|[;|&])/i);
	const script = quotedPython?.[1] ?? plainPython?.[1];
	if (script !== void 0) return pathLeaf(script);
...
function classifyToolError(event) {
	...
	else if (event.exitCode !== null && event.exitCode !== void 0 && event.exitCode !== 0) category = "shell_nonzero";
	return {
		category,
		categoryLabel: ERROR_CATEGORY_LABELS[category] ?? ERROR_CATEGORY_LABELS.tool_error ?? "工具执行失败"
	};
}
function buildCodexEvents(context, records, malformedLines, runRecord) {
	...
	for (const record of records) {
		...
		if (event.type === "item.completed" && item.type === "agent_message" && typeof item.text === "string") {
			...
			assistantOrdinal += 1;
			appendAssistant(log, context, position, startedAt, [{
				type: "text",
				text: item.text
			}], assistantOrdinal);
		...
		if (event.type === "item.completed" && TOOL_ITEM_TYPES.has(item.type)) {
			...
			const exitCode = Number.isInteger(item.exit_code) ? item.exit_code : void 0;
			const isError = item.status === "failed" || exitCode !== void 0 && exitCode !== 0;
			appendTool(log, context, position, {
				...
				isError,
				errorCode: exitCode === void 0 ? "EXTERNAL_TOOL_FAILED" : `EXTERNAL_EXIT_${exitCode}`,
				time: startedAt,
	...
	log.append("turn/end", finishedAt, {
		turn,
		reason: sourceCompleted ? { kind: "completed" } : {
			kind: "aborted",
			reason: { kind: "legacy" }
		}
	});
...
function buildClaudeEvents(context, records, malformedLines, runRecord) {
	...
	function flushAssistant() {
		if (pendingAssistant === void 0) return;
		const pending = pendingAssistant;
		pendingAssistant = void 0;
	...
	for (const record of records) {
		...
		if (event?.type === "assistant" && event.message && typeof event.message === "object") {
			...
			if (pendingAssistant === void 0) throw new Error("Claude assistant state was not initialized");
			const currentAssistant = pendingAssistant;
			currentAssistant.usage = claudeUsage(event.message.usage) ?? currentAssistant.usage;
			for (const block of Array.isArray(event.message.content) ? event.message.content : []) if (block?.type === "thinking") counts.omittedHiddenReasoningEvents += 1;
				...
				const tool = {
					id: requireNonEmptyString(block.id, `Claude tool_use at line ${record.line} id`),
					name: requireNonEmptyString(block.name, `Claude tool_use at line ${record.line} name`),
		...
		if (event?.type === "user" && event.message && typeof event.message === "object") {
			...
			for (const block of Array.isArray(event.message.content) ? event.message.content : []) if (block?.type === "tool_result") {
...
function redactObservableText(value) {
	...
	return (typeof serialized === "string" ? serialized : String(value ?? "")).replace(/((?:api[_-]?key|access[_-]?token|auth[_-]?token|password|secret|authorization)\s*[=:]\s*)([^\s,;]+)/gi, "$1[REDACTED]").replace(/("(?:api[_-]?key|access[_-]?token|auth[_-]?token|password|secret|authorization)"\s*:\s*")[^"]*(")/gi, "$1[REDACTED]$2").replace(/\b(Bearer\s+)[A-Za-z0-9._~+\x2f-]+=*/gi, "$1[REDACTED]");
}
...
function connectToolTransitions(events) {
	...
	for (let index = 0; index < tools.length; index += 1) {
		...
		current.transition = previous === void 0 ? `START → ${current.toolName}` : `${previous.toolName} → ${current.toolName}`;
		const previousEnd = previous?.resultTimestampMs ?? previous?.timestampMs;
		current.gapFromPreviousToolMs = previousEnd !== null && previousEnd !== void 0 && current.timestampMs !== null ? Math.max(0, current.timestampMs - previousEnd) : null;
	}
	for (let index = 0; index < events.length; index += 1) {
		const event = events[index];
		if (event !== void 0) event.seq = index + 1;
	}
	return tools;
}
...
function buildClaudeObservableTrace(context, records, malformedLines, runRecord) {
	...
	for (const record of records) {
		...
		if (event?.type === "assistant" && event.message && typeof event.message === "object") {
			...
			for (const block of Array.isArray(event.message.content) ? event.message.content : []) if (block?.type === "thinking") events.push({
				seq: 0,
...
function liveRunRecord(records, details) {
	const exact = records.flatMap((record) => {
		const value = exactEventTime(record.value?.timestamp ?? record.value?.observed_at ?? record.value?.started_at_utc);
	...
	const finishedExact = records.flatMap((record) => {
		const value = exactEventTime(record.value?.finished_at_utc ?? record.value?.timestamp ?? record.value?.observed_at);
		return value === null ? [] : [value];
	});
...
function nativeLiveSessionId(source, path, caseId) {
	const sessionProjection = source.kind === "generic" ? "v4" : source.kind === "implantagent-trace" ? "v4-2" : "v3-2";
...
function initializeNativeLiveSession(session, source, path, caseId) {
	const state = {
		session,
		...
		callSeqs: /* @__PURE__ */ new Map(),
		callSteps: /* @__PURE__ */ new Map(),
		callLedgerMeta: /* @__PURE__ */ new Map(),
		openCalls: /* @__PURE__ */ new Set(),
		completedCalls: /* @__PURE__ */ new Set(),
		assistantLinkedCalls: /* @__PURE__ */ new Set(),
		pendingCodexAssistants: [],
		pendingClaudeAssistantEvents: [],
		currentClaudeMessageId: null,
		currentGenericRequestId: null,
		seenClaudeBlocks: /* @__PURE__ */ new Set(),
		assistantOrdinal: 0,
		resultOrdinal: 0,
		...
		implantAgentInvocationCounts: /* @__PURE__ */ new Map(),
		ledgerRecords: [],
	...
	session.append("user/message", {
		...
		source: {
			...
			plugin: name,
			form: "notice",
			summary: `Live external trajectory ${caseId}`
		}
	}, { surfaceOp: "append" });
...
function sourceTimestamp(event) {
	const milliseconds = exactEventTime(event?.timestamp ?? event?.observed_at ?? event?.started_at_utc ?? null);
	return {
		milliseconds,
...
function rotateNativeLiveStep(state) {
	...
	state.session.append("step/end", {
		turn: state.turn,
		step: state.step
	});
	state.step += 1;
	state.session.append("step/start", {
		turn: state.turn,
		step: state.step
	});
...
function appendCodexPublicMessage(state, text, record, event, messageId) {
	...
	state.pendingCodexAssistants.push({
		text,
		record,
		event,
		messageId
...
function linkNativeToolToAssistant(state, callId, toolName, argumentsText, record, event, label = null) {
	...
	const content = [];
	if (!state.stepHasAssistantRequest && typeof label === "string" && label !== "") content.push({
		type: "text",
		text: label
	});
	content.push({
...
function prepareCodexToolRequest(state, callId, toolName, argumentsText, record, event) {
	...
	if (state.codexRequestTextBlocks.length === 0) {
		if (state.pendingCodexAssistants.length > 0) for (const pending of state.pendingCodexAssistants.splice(0)) {
			state.codexRequestTextBlocks.push({
				type: "text",
				text: pending.text
			});
		...
		else state.codexRequestTextBlocks.push({
			type: "text",
			text: `Observable Codex request: ${toolName}`
		});
	}
	state.codexRequestToolBlocks.push({
		type: "tool-call",
		id: callId,
		name: toolName,
		arguments: argumentsText
	});
	appendNativeLiveAssistantContent(state, [...state.codexRequestTextBlocks, ...state.codexRequestToolBlocks], record, event, {
		event_type: "request_anchor",
		tool_call_id: callId,
		tool_name: toolName,
		tool_arguments: argumentsText,
...
function prepareImplantAgentModuleToolRequest(state, callId, item, argumentsText, record, event) {
	...
	if (metadata === null) throw new Error(`unregistered ImplantAgent MCP tool cannot be assigned to M1-M6: ${codexRawToolName(item)}`);
	if (state.callSeqs.has(callId)) return {
		metadata,
		audit: state.callLedgerMeta.get(callId) ?? {}
	};
	...
	const requestTitle = dryRun ? "Preflight · 固定工具完整性检查（不属于 M1–M6 临床执行）" : IMPLANTAGENT_MODULES[metadata.moduleId];
...
function flushCodexFinalPublicRequest(state) {
	...
	appendNativeLiveAssistantContent(state, pending.map((item) => ({
		type: "text",
		text: item.text
	})), pending[0]?.record ?? null, pending[0]?.event ?? null);
	for (const item of pending) appendLedgerRecord(state, item.record, item.event, {
		event_type: "assistant_message",
...
function ensureNativeLiveToolCall(state, callId, toolName, argumentsText, record, event, audit = {}) {
	...
	const timestamp = sourceTimestamp(event);
	if (state.lastToolName !== null) appendLedgerRecord(state, record, event, {
	...
	state.callLedgerMeta.set(callId, {
		toolName,
		startedAtMs: timestamp.milliseconds,
		...audit
	});
...
function flushDeferredNativeRequests(state) {
	...
	while (state.pendingClaudeAssistantEvents.length > 0 && state.openCalls.size === 0) {
		...
		if (pending !== void 0) processClaudeAssistantEvent(state, pending.record, pending.event);
	}
}
function appendNativeLiveToolResult(state, callId, text, isError, record, event, durationOverrideMs = null) {
	...
	if (callSeq === void 0) throw new Error(`live native result lacks call ${callId}`);
	...
	state.session.append("tool/result", {
		...
		meta: {
			importedBy: name,
			externalAgent: state.source.kind,
	...
	const measuredDuration = callMeta?.startedAtMs !== null && callMeta?.startedAtMs !== void 0 && resultTimestamp.milliseconds !== null ? Math.max(0, resultTimestamp.milliseconds - callMeta.startedAtMs) : null;
...
function processClaudeAssistantEvent(state, record, event) {
	...
	if (state.currentClaudeMessageId !== null && state.currentClaudeMessageId !== messageId) {
		if (state.openCalls.size > 0) {
			state.pendingClaudeAssistantEvents.push({
				record,
				event
			});
			return;
...
function genericEventType(event, record) {
	if (event?.schema_version !== "external-agent-event-v1") throw new Error(`generic live event at line ${record.line} must use schema_version external-agent-event-v1`);
	return requireNonEmptyString(event.event_type ?? event.type, `generic live event at line ${record.line} event_type`).trim().toLowerCase().replace(/[.-]+/g, "_");
}
function genericRequestId(event, record) {
	return requireNonEmptyString(event.request_id, `generic live event at line ${record.line} request_id`);
}
function genericCallId(state, event, record) {
...
function genericPublicText(event, record) {
	return redactObservableText(requireNonEmptyString(event.public_message ?? event.text, `generic live event at line ${record.line} public_message`));
}
function genericAuditMetadata(event) {
	const metadata = event.metadata !== null && typeof event.metadata === "object" && !Array.isArray(event.metadata) ? event.metadata : {};
	const optionalString = (key) => {
		const value = event[key] ?? metadata[key];
		return typeof value === "string" && value.trim() !== "" ? value : null;
	};
	const optionalIndex = (key) => {
		const value = event[key] ?? metadata[key];
...
function flushGenericRequestWithoutTool(state, record, event) {
...
function beginGenericRequest(state, record, event) {
	const requestId = genericRequestId(event, record);
	if (state.currentGenericRequestId === requestId) throw new Error(`generic request_started repeated request_id ${requestId}`);
	...
	state.codexRequestTextBlocks = [{
		type: "text",
		text: genericPublicText(event, record)
	}];
	state.codexRequestToolBlocks = [];
	appendLedgerRecord(state, record, event, {
...
function assertGenericRequest(state, record, event) {
	const requestId = genericRequestId(event, record);
	if (state.currentGenericRequestId !== requestId) throw new Error(`generic event at line ${record.line} references request ${requestId}; active request is ${String(state.currentGenericRequestId)}`);
}
function appendGenericAssistantMessage(state, record, event) {
...
function appendGenericToolCall(state, record, event) {
	assertGenericRequest(state, record, event);
...
function appendGenericToolResult(state, record, event) {
	assertGenericRequest(state, record, event);
	const callId = genericCallId(state, event, record);
	...
	const duration = typeof event.duration_ms === "number" && Number.isFinite(event.duration_ms) && event.duration_ms >= 0 ? event.duration_ms : null;
	appendNativeLiveToolResult(state, callId, redactObservableText(event.result ?? event.output ?? ""), isError, record, event, duration);
}
function appendGenericRequestCompleted(state, record, event) {
	assertGenericRequest(state, record, event);
...
function implantAgentRequestId(event, record) {
	return requireNonEmptyString(event.request_id, `ImplantAgent event at line ${record.line} request_id`);
}
function assertImplantAgentRequest(state, record, event) {
	const requestId = implantAgentRequestId(event, record);
	if (state.currentGenericRequestId !== requestId) throw new Error(`ImplantAgent event at line ${record.line} references request ${requestId}; active request is ${String(state.currentGenericRequestId)}`);
}
function implantAgentModuleText(event) {
	const moduleId = typeof event.module_id === "string" ? event.module_id.toUpperCase() : "UNKNOWN";
	const title = IMPLANTAGENT_MODULES[moduleId] ?? `${moduleId} · External module`;
	const allowed = Array.isArray(event.allowed_tool_ids) ? event.allowed_tool_ids.map(String).join(", ") : "";
	return allowed === "" ? title : `${title}\n当前允许节点：${allowed}`;
}
function beginImplantAgentRequest(state, record, event) {
	const requestId = implantAgentRequestId(event, record);
	if (state.currentGenericRequestId === requestId) throw new Error(`ImplantAgent request_started repeated request_id ${requestId}`);
	if (state.openCalls.size > 0) throw new Error(`ImplantAgent request ${requestId} started with open tool calls`);
	if (state.currentGenericRequestId !== null) {
		flushGenericRequestWithoutTool(state, record, event);
		rotateNativeLiveStep(state);
	}
	...
	state.codexRequestTextBlocks = [{
		type: "text",
		text: implantAgentModuleText(event)
	}];
	state.codexRequestToolBlocks = [];
	appendLedgerRecord(state, record, event, {
		event_type: "request_started",
		public_assistant_message: implantAgentModuleText(event),
		status: "in_progress",
		tool_call_id: requestId,
		module_id: typeof event.module_id === "string" ? event.module_id : null
	});
}
function appendImplantAgentAction(state, record, event) {
	assertImplantAgentRequest(state, record, event);
	const action = typeof event.action === "string" ? event.action : "observable_action";
	const toolId = typeof event.tool_id === "string" ? event.tool_id.toUpperCase() : null;
	const siteId = typeof event.site_id === "string" ? event.site_id : null;
	const publicReason = typeof event.public_reason === "string" && event.public_reason.trim() !== "" ? redactObservableText(event.public_reason) : "No public reason was emitted by the external supervisor.";
	const actionText = [`公开动作：${action}${toolId === null ? "" : ` · ${toolId}`}${siteId === null ? "" : ` · site ${siteId}`}`, `公开理由：${publicReason}`].join("\n");
	state.codexRequestTextBlocks.push({
		type: "text",
		text: actionText
	});
	appendLedgerRecord(state, record, event, {
		event_type: "assistant_message",
		public_assistant_message: actionText,
		status: "completed",
		tool_call_id: state.currentGenericRequestId,
		module_id: typeof event.module_id === "string" ? event.module_id : null,
		node_id: toolId
...
function implantAgentVisibleToolName(toolId) {
	const title = IMPLANTAGENT_NODE_TITLES[toolId];
	return title === void 0 ? toolId : `${toolId} · ${title}`;
}
function implantAgentObservableCallId(event, record) {
	const raw = requireNonEmptyString(event.call_id, `ImplantAgent tool event at line ${record.line} call_id`);
	return `${sanitizeId(raw) || sha256Text(raw).slice(0, 12)}${Number.isSafeInteger(event.node_revision) ? `-revision-${event.node_revision}` : ""}${Number.isSafeInteger(event.retry_index) ? `-retry-${event.retry_index}` : ""}`;
}
function implantAgentCallId(state, event, record) {
	return `external-live-implantagent-${sanitizeId(state.source.id)}-${implantAgentObservableCallId(event, record)}`;
}
function implantAgentAuditMetadata(event) {
	const toolId = typeof event.tool_id === "string" ? event.tool_id.toUpperCase() : null;
	const rawToolName = typeof event.raw_tool_name === "string" && event.raw_tool_name.trim() !== "" ? event.raw_tool_name : toolId;
	return {
		module_id: typeof event.module_id === "string" ? event.module_id : null,
		node_id: toolId,
		raw_tool_name: rawToolName,
		invocation_index: Number.isSafeInteger(event.invocation_count) ? event.invocation_count : null,
		retry_index: Number.isSafeInteger(event.retry_index) ? event.retry_index : null
	};
}
function appendImplantAgentToolCall(state, record, event) {
	assertImplantAgentRequest(state, record, event);
	const callId = implantAgentCallId(state, event, record);
	if (state.callSeqs.has(callId)) return;
	const toolName = implantAgentVisibleToolName(requireNonEmptyString(event.tool_id, `ImplantAgent tool_call at line ${record.line} tool_id`).toUpperCase());
	const argumentsText = redactObservableText(event.arguments_summary ?? {});
	const audit = implantAgentAuditMetadata(event);
	state.codexRequestToolBlocks = [{
		type: "tool-call",
		id: callId,
		name: toolName,
...
function appendImplantAgentToolResult(state, record, event) {
	assertImplantAgentRequest(state, record, event);
	const callId = implantAgentCallId(state, event, record);
	const status = String(event.status ?? "").trim().toLowerCase();
	const isError = ![
	...
	appendNativeLiveToolResult(state, callId, redactObservableText(isError ? {
		status: event.status ?? null,
		error_type: event.error_type ?? null,
		error_message: event.error_message ?? null,
		node_revision: event.node_revision ?? null
	} : {
		status: event.status ?? null,
		result_summary: event.result_summary ?? null,
		result_sha256: event.result_sha256 ?? null,
		output_artifact_handles: event.output_artifact_handles ?? [],
		node_revision: event.node_revision ?? null
	}), isError, record, event, typeof event.duration_ms === "number" ? event.duration_ms : null);
}
function finalizeNativeLiveSession(state, succeeded, message, record, event) {
	if (state.finalized) return;
	for (const callId of state.openCalls) appendNativeLiveToolResult(state, callId, `External run terminated before a tool result was observed: ${message}`, true, record, event);
	flushDeferredNativeRequests(state);
	if (state.source.kind === "codex" && state.source.projectionMode !== "implantagent-modules") flushCodexFinalPublicRequest(state);
	if (state.source.kind === "generic" || state.source.kind === "implantagent-trace") flushGenericRequestWithoutTool(state, record, event);
	if (state.lastToolName !== null) appendLedgerRecord(state, record, event, {
		event_type: "tool_transition",
		status: "terminal",
		previous_tool: state.lastToolName,
	...
	state.session.append("turn/end", {
		turn: state.turn,
		reason: succeeded ? { kind: "completed" } : {
			kind: "error",
			error: {
				message,
				code: "EXTERNAL_RUN_FAILED"
			}
		}
	});
...
function appendNativeLiveRecords(state, records) {
	let appended = 0;
	for (const record of records) {
		...
		if (state.source.kind === "codex") {
			...
			if (item === null || typeof item !== "object") continue;
			if (item.type === "todo_list" && Array.isArray(item.items)) {
				state.session.append("todo/write", { todos: item.items.map((entry) => ({
					content: typeof entry?.text === "string" ? entry.text : JSON.stringify(entry),
					status: entry?.completed === true ? "completed" : "pending"
				})) });
				appended += 1;
			...
			if (event.type === "item.completed" && item.type === "agent_message") {
				if (state.source.projectionMode === "implantagent-modules") appendLedgerRecord(state, record, event, {
					event_type: "assistant_message",
					public_assistant_message: item.text,
					status: "completed",
				...
				else appendCodexPublicMessage(state, item.text, record, event, item.id ?? `line-${record.line}`);
				appended += 1;
				continue;
			}
			if (TOOL_ITEM_TYPES.has(item.type) && (event.type === "item.started" || event.type === "item.completed")) {
				const callId = `external-live-codex-${sanitizeId(typeof item.id === "string" && item.id !== "" ? item.id : `line-${record.line}`)}`;
				...
				} else prepareCodexToolRequest(state, callId, visibleToolName, argumentsText, record, event);
				...
				if (event.type === "item.completed") {
					const exitCode = Number.isInteger(item.exit_code) ? item.exit_code : void 0;
					const failed = item.status === "failed" || exitCode !== void 0 && exitCode !== 0;
		...
		if (state.source.kind === "claude") {
			...
			if (event?.type === "result") {
				finalizeNativeLiveSession(state, event.subtype === "success" && event.is_error !== true && event.terminal_reason !== "api_error", `External Claude terminal subtype: ${String(event.subtype ?? "unknown")}`, record, event);
				appended += 3;
			}
			continue;
		}
		if (state.source.kind === "implantagent-trace") {
			...
			if (type === "request_started") {
				beginImplantAgentRequest(state, record, event);
				appended += 1;
				continue;
			}
			if (type === "action_selected") {
				appendImplantAgentAction(state, record, event);
				appended += 1;
				continue;
			}
			if (type === "tool_call") {
				appendImplantAgentToolCall(state, record, event);
				appended += 2;
				continue;
			}
			if (type === "tool_result") {
				appendImplantAgentToolResult(state, record, event);
				appended += 1;
				continue;
			}
			if (type === "supervisor_failure") {
				assertImplantAgentRequest(state, record, event);
				const message = `公开故障：${String(event.error_type ?? "SupervisorFailure")} · ${String(event.error_message ?? "No public error message")}`;
				state.codexRequestTextBlocks.push({
					type: "text",
					text: redactObservableText(message)
				});
				appendLedgerRecord(state, record, event, {
					event_type: "supervisor_failure",
					public_assistant_message: redactObservableText(message),
					status: "failed",
					tool_call_id: state.currentGenericRequestId,
					module_id: typeof event.module_id === "string" ? event.module_id : null
				});
			...
			if (type === "run_finished") {
				const succeeded = String(event.status ?? "").trim().toLowerCase() === "completed";
				finalizeNativeLiveSession(state, succeeded, typeof event.finish_reason === "string" && event.finish_reason !== "" ? event.finish_reason : `ImplantAgent run ${succeeded ? "completed" : "failed"}`, record, event);
				appended += 3;
				continue;
			}
			appendLedgerRecord(state, record, event, {
				event_type: type === "" ? "implantagent_event" : type,
				status: typeof event.status === "string" ? event.status : "observed",
				tool_call_id: typeof event.call_id === "string" ? event.call_id : null,
				module_id: typeof event.module_id === "string" ? event.module_id : null,
				node_id: typeof event.tool_id === "string" ? event.tool_id : null
			});
			appended += 1;
			continue;
		}
		if (state.source.kind === "generic") {
			...
			if (type === "run_completed" || type === "run_failed") {
				const failed = type === "run_failed" || [
					"error",
					"failed",
					"failure"
				].includes(String(event.status ?? "").toLowerCase());
				finalizeNativeLiveSession(state, !failed, typeof event.message === "string" && event.message !== "" ? redactObservableText(event.message) : `External generic run ${failed ? "failed" : "completed"}`, record, event);
				appended += 3;
				continue;
			}
...
function unflushedNativeLedgerRecords(state) {
	return state.ledgerRecords.slice(state.ledgerFlushedCount);
}
function markNativeLedgerFlushed(state) {
	state.ledgerFlushedCount = state.ledgerRecords.length;
}
function buildGenericObservableTrace(context, records, malformedLines, runRecord) {
	const events = [];
	const toolsById = /* @__PURE__ */ new Map();
	let publicContext = "";
	let phase = 0;
	...
	for (const record of records) {
		...
		if (type === "tool_result") {
			...
			const tool = toolsById.get(callId);
			if (tool === void 0) throw new Error(`generic tool_result at line ${record.line} has no preceding tool_call ${callId}`);
			const status = String(event.status ?? "").trim().toLowerCase();
			...
			tool.durationMs = typeof event.duration_ms === "number" && Number.isFinite(event.duration_ms) && event.duration_ms >= 0 ? event.duration_ms : tool.timestampMs !== null && timestampMs !== null ? Math.max(0, timestampMs - tool.timestampMs) : null;
...
function buildImplantAgentToolTrace(context, records, malformedLines, runRecord) {
	if (records.some((record) => typeof record.value?.event_type === "string")) {
		const events = [];
		const toolsById = /* @__PURE__ */ new Map();
		let phase = 0;
		let publicContext = "";
		...
		for (const record of records) {
			...
			const timestampMs = exactEventTime(row?.observed_at);
			if (timestampMs === null) missingPerEventTimestamps += 1;
			if (type === "request_started") {
				phase += 1;
				publicContext = implantAgentModuleText(row);
				events.push({
					seq: 0,
					kind: "public_plan",
					phase,
					...
					timeEvidence: timestampMs === null ? "not-recorded" : "source-event",
					sourceLine: record.line,
					requestId: row.request_id ?? `line-${record.line}`,
					text: publicContext,
					preview: previewText(publicContext)
				});
				continue;
			}
			if (type === "action_selected" || type === "supervisor_failure") {
				const action = type === "action_selected" ? `Action ${String(row.action ?? "unknown")} ${String(row.tool_id ?? "")}`.trim() : `Supervisor failure ${String(row.error_type ?? "")}`.trim();
				const reason = type === "action_selected" ? row.public_reason : row.error_message;
				publicContext = `${action}\n${String(reason ?? "")}`.trim();
				events.push({
					seq: 0,
					kind: "public_reasoning",
					phase,
			...
			if (type === "tool_call") {
				...
				const tool = {
					...
					gapFromPreviousToolMs: null,
					callId: rawCallId,
					toolName: implantAgentVisibleToolName(toolId),
					rawToolName: audit.raw_tool_name ?? toolId,
					workflowNodeId: toolId,
					workflowModuleId: audit.module_id ?? null,
					invocationIndex: audit.invocation_index ?? null,
					retryIndex: audit.retry_index ?? null,
					status: "pending",
			...
			if (type === "tool_result") {
				...
				const resultText = redactObservableText(failed ? {
				...
				tool.result = resultText;
				tool.resultPreview = previewText(resultText);
				tool.durationMs = typeof row.duration_ms === "number" && Number.isFinite(row.duration_ms) ? Math.max(0, row.duration_ms) : tool.timestampMs !== null && timestampMs !== null ? Math.max(0, timestampMs - tool.timestampMs) : null;
				if (failed) Object.assign(tool, classifyToolError(tool));
			}
		}
		return makeObservableTrace(context, runRecord, malformedLines, "source-event-timestamps", events, {
	...
	for (const record of records) {
		...
		events.push({
			...
			resultTimestampMs,
			timeEvidence: timestampMs === null ? "not-recorded" : "source-event",
			durationMs: Number.isFinite(row.duration_ms) ? Math.max(0, Number(row.duration_ms)) : timestampMs !== null && resultTimestampMs !== null ? Math.max(0, resultTimestampMs - timestampMs) : null,
			gapFromPreviousToolMs: null,
	...
	return makeObservableTrace(context, runRecord, malformedLines, "source-event-timestamps", events, { hiddenReasoningContentIncluded: 0 });
}
function emptyLiveMonitorTrace(liveSources) {
	...
	return {
		...
		source: {
			path: liveSources.map((source) => source.root).join(" | "),
			malformedLines: [],
			timeCoverage: "mixed-live-sources"
		},
		run: {
			startedAtMs: now,
			finishedAtMs: now,
		...
		stats: {
			observableEvents: 0,
			toolCalls: 0,
			publicReasoningEvents: 0,
			publicPlanEvents: 0,
			privateReasoningMarkers: 0,
			successfulTools: 0,
			failedTools: 0,
			exactToolTimestamps: 0,
			exactToolDurations: 0,
			liveStreams: 0,
			monitoredStreams: liveSources.length
		},
		events: [],
		live: false,
		streams: []
...
async function buildLiveMonitorTrace(liveManifest) {
	...
	for (const source of sources) {
		...
		const context = {
			source: {
				agent: source.kind === "implantagent-trace" ? "implantagent" : source.kind,
				caseId,
		...
		const trace = source.kind === "codex" ? buildCodexObservableTrace(context, jsonl.records, jsonl.malformedLines, runRecord) : source.kind === "claude" ? buildClaudeObservableTrace(context, jsonl.records, jsonl.malformedLines, runRecord) : source.kind === "generic" ? buildGenericObservableTrace(context, jsonl.records, jsonl.malformedLines, runRecord) : buildImplantAgentToolTrace(context, jsonl.records, jsonl.malformedLines, runRecord);
		const isLive = Date.now() - latest.details.mtimeMs < 15e3;
	...
	return {
		...
		errorComparison: traces.map((item) => {
			...
			for (const event of errors) {
				const classified = event.errorCategory === void 0 ? classifyToolError(event) : {
			...
			return {
				...
				errorTools: [...counts].map(([toolName, count]) => ({
					toolName,
					count
				})).sort((left, right) => right.count - left.count || left.toolName.localeCompare(right.toolName)),
				errorCategories: [...categoryCounts].map(([label, count]) => ({
					label,
					count
				})).sort((left, right) => right.count - left.count || left.label.localeCompare(right.label)),
				firstError: first === void 0 ? null : diagnostic(first),
				diagnostics: errors.map(diagnostic)
			};
		})
	};
}
function buildMonitorStats(trace, rawArgs = {}) {
	if (rawArgs === null || typeof rawArgs !== "object" || Array.isArray(rawArgs)) throw new Error("trajectory_stats arguments must be an object");
	const args = rawArgs;
	for (const key of Object.keys(args)) if (![
		"agent",
		"case_id",
		"tool_name",
		"include_errors"
	].includes(key)) throw new Error(`trajectory_stats does not accept argument ${key}`);
	for (const key of [
		"agent",
		"case_id",
		"tool_name"
	]) if (args[key] !== void 0 && typeof args[key] !== "string") throw new Error(`${key} must be a string when provided`);
	if (args.include_errors !== void 0 && typeof args.include_errors !== "boolean") throw new Error("include_errors must be a boolean when provided");
	...
	const arms = selectedId === "" || selectedId === "all" ? comparisons : comparisons.filter((arm) => [
		arm.id,
		arm.kind,
		arm.label
	].some((value) => String(value).toLowerCase() === selectedId));
	...
	const summaries = arms.map((arm) => {
		...
		const caseTools = caseFilter === "" ? armTools : armTools.filter((event) => String(event.streamCaseId ?? "").toLowerCase() === caseFilter);
		const tools = toolFilter === "" ? caseTools : caseTools.filter((event) => [event.toolName, event.rawToolName].some((value) => String(value ?? "").toLowerCase().includes(toolFilter)));
		...
		for (const event of tools) {
			const toolName = event.toolName ?? event.rawToolName ?? "unknown";
			const current = toolCounts.get(toolName) ?? {
				toolName,
				calls: 0,
				success: 0,
				error: 0,
		...
		return {
			...
			...args.include_errors === true ? { errorExamples: errors.slice(0, 10).map((event) => ({
				toolName: event.toolName,
...
async function monitorControlToolGuard(exec, next) {
	...
	if (exec.agent?.session?.id === LIVE_MONITOR_SESSION_ID && (toolName === void 0 || !MONITOR_CONTROL_TOOLS.has(toolName))) return {
		kind: "deny",
		reason: "The combined monitor session permits only catalog, start, status and trajectory statistics tools."
	};
	return next();
}
async function prepareImport(source) {
	...
	const sha256 = sha256Text(jsonl.text);
	if (sha256 !== source.expectedSha256) throw new Error(`source hash mismatch for ${source.sourcePath}: expected ${source.expectedSha256}, got ${sha256}`);
	...
	const converted = source.agent === "codex" ? buildCodexEvents(context, jsonl.records, jsonl.malformedLines, runRecord) : buildClaudeEvents(context, jsonl.records, jsonl.malformedLines, runRecord);
	const observableTrace = source.agent === "codex" ? buildCodexObservableTrace(context, jsonl.records, jsonl.malformedLines, runRecord) : buildClaudeObservableTrace(context, jsonl.records, jsonl.malformedLines, runRecord);
	return {
		source,
		sessionId,
		sha256,
...
function validateConfig(config) {
	...
	if (config === null || typeof config !== "object" || Array.isArray(config)) throw new Error("external trajectory importer config must be an object");
	const candidate = config;
	const manifestPath = candidate.manifestPath === void 0 ? DEFAULT_MANIFEST_PATH : requireNonEmptyString(candidate.manifestPath, "manifestPath");
	const liveManifestPath = candidate.liveManifestPath === void 0 ? DEFAULT_LIVE_MANIFEST_PATH : requireNonEmptyString(candidate.liveManifestPath, "liveManifestPath");
	const reportPath = candidate.reportPath === void 0 ? void 0 : requireNonEmptyString(candidate.reportPath, "reportPath");
	const enableLaunchTools = candidate.enableLaunchTools === true;
	const launchPlansPath = candidate.launchPlansPath === void 0 ? DEFAULT_LAUNCH_PLANS_PATH : requireNonEmptyString(candidate.launchPlansPath, "launchPlansPath");
	const runtimeRegistrationRoot = candidate.runtimeRegistrationRoot === void 0 ? DEFAULT_RUNTIME_REGISTRATION_ROOT : requireNonEmptyString(candidate.runtimeRegistrationRoot, "runtimeRegistrationRoot");
	const runRegistryRoot = candidate.runRegistryRoot === void 0 ? DEFAULT_RUN_REGISTRY_ROOT : requireNonEmptyString(candidate.runRegistryRoot, "runRegistryRoot");
	if (!isAbsolute(manifestPath) || !isAbsolute(liveManifestPath) || reportPath !== void 0 && !isAbsolute(reportPath) || !isAbsolute(launchPlansPath) || !isAbsolute(runtimeRegistrationRoot) || !isAbsolute(runRegistryRoot)) throw new Error("all configured manifest, report, launch and runtime registry paths must be absolute");
	return {
		manifestPath: resolve(manifestPath),
		liveManifestPath: resolve(liveManifestPath),
		enableLaunchTools,
		...reportPath === void 0 ? {} : { reportPath: resolve(reportPath) },
		...enableLaunchTools ? {
...
async function apply(ctx, rawConfig) {
	...
	const launchManager = config.enableLaunchTools && config.launchPlansPath !== void 0 && config.runtimeRegistrationRoot !== void 0 && config.runRegistryRoot !== void 0 ? new PreexperimentLaunchManager({
		plansPath: config.launchPlansPath,
		registrationRoot: config.runtimeRegistrationRoot,
		runRegistryRoot: config.runRegistryRoot
	}) : null;
	const launchPlans = launchManager === null ? null : await loadLaunchPlans(config.launchPlansPath);
	...
	ctx.tools.register({
		...
		parameters: {
			...
			properties: {
				agent: {
					type: "string",
					description: "Optional: all, or any configured source id, kind, or label."
				},
				case_id: {
					type: "string",
					description: "Optional exact case id from the current live stream."
				},
				tool_name: {
					type: "string",
					description: "Optional case-insensitive tool-name substring, such as jq or build_case_outputs.py."
				},
				include_errors: {
					type: "boolean",
					description: "Include up to 10 bounded error examples; false by default."
				}
			},
	...
	if (launchManager !== null) {
		ctx.tools.register({
			...
			presentCall: () => ({
				card: "generic",
				title: "List external preexperiment plans",
				kind: "read"
			}),
			async execute() {
		...
		ctx.tools.register({
			...
			parameters: {
				...
				properties: {
					plan_id: {
						type: "string",
						description: "Exact enabled plan ID returned by external_preexperiment_catalog."
					},
					case_id: {
						type: "string",
						description: "Exact allowlisted case ID returned by external_preexperiment_catalog."
					},
					confirmation: {
						type: "string",
						enum: ["START_EXTERNAL_PREEXPERIMENT"],
						description: "Required exact confirmation string."
					}
				},
				required: [
					"plan_id",
		...
		ctx.tools.register({
			...
			parameters: {
				...
				properties: {
					run_id: {
						type: "string",
						description: "Optional exact managed run ID."
					},
					plan_id: {
						type: "string",
						description: "Optional exact launch plan ID."
					},
					case_id: {
						type: "string",
			...
			presentCall: () => ({
				card: "generic",
				title: "Read external preexperiment status",
				kind: "read"
			}),
			async execute(args) {
	...
	const traceBodies = /* @__PURE__ */ new Map();
	let liveCache = {
		expiresAt: 0,
		body: ""
	};
	...
	const nativeStates = /* @__PURE__ */ new Map();
		...
		try {
			...
			for (const source of liveManifest.sources.filter((item) => item.nativeSession)) {
				...
				if (state === void 0) {
					...
					state = initializeNativeLiveSession(ctx.sessions.create(sessionId, { meta: {
						cwd: source.cwd,
						createdAt: Math.max(0, Math.floor(latest.details.birthtimeMs || latest.details.ctimeMs || Date.now()))
					} }), source, latest.path, caseId);
	...
	} else report.liveMonitorStatus = "already-created";
	...
	if (config.reportPath !== void 0) await writeFile(config.reportPath, `${JSON.stringify(report, null, 2)}\n`, "utf8");
	ctx.logger.info(`external trajectory importer: ${report.sessions.length} source session(s) checked; no model was run`);
}
//#endregion
export { LIVE_MONITOR_SESSION_ID, NATIVE_LIVE_PROJECTION_VERSION, NORMALIZED_LEDGER_SCHEMA, PreexperimentLaunchManager, appendNativeLiveRecords, apply, buildLiveMonitorTrace, buildMonitorStats, deriveSessionId, initializeNativeLiveSession, inject, isImportedSessionId, liveMonitorSeed, loadCombinedLiveManifest, loadLaunchPlans, loadLiveManifest, loadManifest, loadRuntimeRegistrations, markNativeLedgerFlushed, monitorControlToolGuard, name, nativeLiveSessionId, prepareImport, readonlyImportedSessionGuard, unflushedNativeLedgerRecords, validateLiveSource };
