...
const LIVE_MONITOR_SESSION_ID = `${SESSION_PREFIX}live-monitor-v3-2`;
const NATIVE_LIVE_PROJECTION_VERSION = "4.2.0";
...
function validateConfig(config) {
	...
	if (!isAbsolute(manifestPath) || !isAbsolute(liveManifestPath) || reportPath !== void 0 && !isAbsolute(reportPath)) throw new Error("manifestPath, liveManifestPath and reportPath must be absolute when configured");
	return {
...
function liveMonitorSeed(time) {
	...
	log.append("user/message", time, {
		...
		content: [{
			type: "text",
			text: "Read-only live monitor for any configured external agent public Request and tool trajectory. Codex and Claude use native adapters; other processes use external-agent-event-v1 JSONL. No model is run by this session."
		}],
		source: {
			kind: "plugin",
			plugin: name,
			form: "notice",
			summary: "External agents live monitor"
		}
	}, { surfaceOp: "append" });
	log.append("session/title", time, {
...
//#endregion
export { LIVE_MONITOR_SESSION_ID, NATIVE_LIVE_PROJECTION_VERSION, NORMALIZED_LEDGER_SCHEMA, appendNativeLiveRecords, apply, buildLiveMonitorTrace, buildMonitorStats, deriveSessionId, initializeNativeLiveSession, inject, isImportedSessionId, liveMonitorSeed, loadLiveManifest, loadManifest, markNativeLedgerFlushed, name, nativeLiveSessionId, prepareImport, readonlyImportedSessionGuard, unflushedNativeLedgerRecords };
