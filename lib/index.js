...
const NATIVE_LIVE_PROJECTION_VERSION = "4.0.0";
...
function sourceTimestamp(event) {
	const milliseconds = exactEventTime(event?.timestamp ?? event?.started_at_utc ?? null);
	return {
		milliseconds,
	...
	const nativeTick = async () => {
		...
		try {
			for (const source of liveManifest.sources.filter((item) => item.nativeSession && [
				...
				const trace = source.kind === "codex" ? buildCodexObservableTrace(context, jsonl.records, jsonl.malformedLines, runRecord) : source.kind === "claude" ? buildClaudeObservableTrace(context, jsonl.records, jsonl.malformedLines, runRecord) : buildGenericObservableTrace(context, jsonl.records, jsonl.malformedLines, runRecord);
	...
	ctx.effect(() => ctx.webServer.register({
		...
		async handler(req, res) {
			...
			if (sessionId === LIVE_MONITOR_SESSION_ID) {
				const now = Date.now();
				if (liveCache.expiresAt <= now) liveCache = {
					expiresAt: now + 1500,
