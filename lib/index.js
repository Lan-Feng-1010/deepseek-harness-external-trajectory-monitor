...
const NATIVE_LIVE_PROJECTION_VERSION = "3.2.0";
const NORMALIZED_LEDGER_SCHEMA = "external-trajectory-ledger-v2";
...
function validateLiveSource(source, index) {
	...
	return {
		...
		provider: typeof source.provider === "string" && source.provider !== "" ? source.provider : kind === "claude" ? "claude-cli" : "codex-cli",
		model: typeof source.model === "string" && source.model !== "" ? source.model : kind === "claude" ? "external-claude" : "external-codex",
...
async function buildLiveMonitorTrace(liveManifest) {
	...
	for (const source of sources) {
		...
		const trace = source.kind === "codex" ? buildCodexObservableTrace(context, jsonl.records, jsonl.malformedLines, runRecord) : source.kind === "claude" ? buildClaudeObservableTrace(context, jsonl.records, jsonl.malformedLines, runRecord) : buildImplantAgentToolTrace(context, jsonl.records, jsonl.malformedLines, runRecord);
...
function buildMonitorStats(trace, rawArgs = {}) {
	...
	if (arms.length === 0) throw new Error(`unknown agent ${args.agent}; use all, implantagent, codex, or claude`);
...
function liveMonitorSeed(time) {
	...
	log.append("user/message", time, {
		...
		content: [{
			type: "text",
			text: "Read-only live monitor for external Codex, Claude and ImplantAgent public tool trajectories. No model is run by this session."
		}],
		source: {
			kind: "plugin",
			plugin: name,
	...
	ctx.tools.register({
		...
		parameters: {
			...
			properties: {
				agent: {
					type: "string",
					description: "Optional: all, implantagent, codex, or claude."
				},
				case_id: {
					type: "string",
	...
	const nativeTick = async () => {
		...
		try {
			for (const source of liveManifest.sources.filter((item) => item.nativeSession && ["codex", "claude"].includes(item.kind))) {
				...
				const trace = source.kind === "codex" ? buildCodexObservableTrace(context, jsonl.records, jsonl.malformedLines, runRecord) : buildClaudeObservableTrace(context, jsonl.records, jsonl.malformedLines, runRecord);
