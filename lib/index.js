...
var PreexperimentLaunchManager = class {
	...
	async start(args) {
		...
		try {
			...
			if ((await loadRunRecords(this.options.runRegistryRoot)).filter((record) => record.planId === plan.id && record.state === "running" && processAlive(record.processId) === true).length >= plan.maxConcurrentRuns) throw new Error(`launch plan ${plan.id} reached its concurrent-run limit`);
